...
export function Stats({ stats }: { stats: SiteStat[] }) {
  ...
  return (
    <section className="border-b border-hairline bg-cream/45" aria-label="DENGLER at a glance">
      <div className="shell py-16 md:py-20">
        <Reveal className="mb-12 flex items-center gap-4">
          <span className="eyebrow">Trusted by investors across 24 markets</span>
        ...
        <RevealGroup className="grid grid-cols-2 gap-x-8 gap-y-12 lg:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.id} className="border-t border-ink/12 pt-6">
              <p className="font-display text-[clamp(2.5rem,5vw,4rem)] leading-none text-ink">
                {stat.value}
              </p>
