...
import { INQUIRY_STATUSES, type Inquiry } from "@/lib/types";
    ...
    <li className={`py-5 transition-opacity ${isPending ? "opacity-50" : ""}`}>
      <div className="flex flex-wrap items-start gap-x-6 gap-y-3">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          >
            {expanded ? t.admin.hide : t.admin.message}
          </button>
          ...
          <button
            ...
            disabled={isPending}
            onClick={() => {
              if (!confirm(fill(t.admin.deleteInquiryConfirm, { name: inquiry.name })))
                return;
              startTransition(() => {
                void deleteInquiry(inquiry.id);
              });
            }}
            aria-label={fill(t.admin.deleteEnquiryAria, { name: inquiry.name })}
            className="rounded-full border border-hairline px-3 py-1.5 text-[11px] uppercase tracking-[0.14em] text-muted transition-colors hover:border-plum hover:text-plum rtl:tracking-normal rtl:normal-case"
          >
            {t.admin.delete}
          </button>
        </div>
